...
2. 通过命令行修改参数。如果不想修改`config/config.yaml`文件，也可以在命令行中修改config参数，命令如下：
   ...
   ```shell
   snakemake consensus --cores 12 --config input_fastq=the/path/to/fastq sample_name=today min_read_len=500	# 相当于修改了config.yaml文件中的inpuf_fastq、sample_name和min_read_len 的值
   
   snakemake annotate --cores 3 --config input_fastq=the/path/to/fastq sample_name=today min_read_len=500	
   
   snakemake report --cores 1 --config input_fastq=the/path/to/fastq sample_name=today min_read_len=500	
   ```
...
3. 指定输出目录，或者在任何目录下进行分析
   ...
   4. 对于环形的病毒基因组组可能会有问题
   5. 增加其他病毒的注释信息，目前只能用于其他病毒获得一致性序列即只执行`snakemake consensus`和`snakemake report`，而没有注释信息`snakemake annotate`
