...
def plot_coverage(per_base_dp, out_dir, sample_name):
    ...
    for group in grouped.groups:
        ...
        plt.savefig(path.join(out_dir, f"{sample_name}.coverage.pdf"))


if __name__ == '__main__':
    usage = f"usage: {path.basename(__file__)} <per_base_dp.bed> <out_fig_path> <sample_name>"
    if len(argv) != 4:
        raise Exception(usage + "\n")
    per_base_dp = argv[1]
    out_dir = argv[2]
    sample = argv[3]
    plot_coverage(per_base_dp, out_dir, sample)
