...
def get_args():
    parser = argparse.ArgumentParser(prog="Get snv depends on the depth of each bases at each position, then "
    ...
    parser.add_argument('--snv_freq', type=float, required=True, help="The min allele freq of alt base for a snv")
    parser.add_argument('--outfile', type=str, required=True, help="Output the variants into this vcf file")
    ...
    parser.add_argument('low_cover_bed', type=str, help="The low coverage region bed file")
    parser.add_argument('ppebd', type=str,
...
def get_indel_info(indel: str) -> Dict[str, Tuple[List[str], List[int]]]:
    with gzip.open(indel, 'rt') if indel.endswith(".gz") else open(indel, 'r') as infile:
        d = {}
        for line in infile:
            if line.startswith("#"):
                continue
...
def dont_check_snv_region(
        low_cover_bed: str,
        indel_info: Dict[str, Tuple[List[str], List[int]]]) -> Dict[str, List[int]]:
    """
...
def merge_snv_and_indel(
    ...
    columns = ['contig', 'pos', 'id', 'ref', 'alt', 'qual', 'filter', 'info', 'format', 'sample']
    variants = [indel[0] for indel in list(indel_info.values())]

    for contig, ctg_no_snv_region in no_snv_region.items():
        sub_df = ppebd_df.query("(contig == @contig) & (pos not in @ctg_no_snv_region)").reset_index(drop=True)
        freq_df = sub_df[list('acgt')]
        criteria1 = freq_df.T.idxmax() != sub_df[
            'ref'].str.lower()  # if the most dp of base is not ref_base, it may be a mutation
        ...
        candidate_snvs.insert(
            len(candidate_snvs.columns), 'dp',
            candidate_snvs['a'] + candidate_snvs['t'] + candidate_snvs['g'] + candidate_snvs['c']
        )
